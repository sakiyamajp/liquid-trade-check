...
function drawTable(ds) {
  ...
  let trAll = d3.select("tbody").selectAll("tr").data(ds, d => d.id);
  trAll.exit().remove();
  let trEnter = trAll.enter().append("tr");
  for (let name of keys) {
    trEnter.append("td").attr('class', "table_" + name);
  }
  trEnter = trEnter.merge(trAll);
  for (let name of keys) {
    trEnter.select("td.table_" + name).text(function (d) {
